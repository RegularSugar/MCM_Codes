"""将第一问 MILP 的 CSV 结果写入官方 result1.xlsx 模板。"""
...
def main() -> None:
    ...
    if len(purchases) != 144 or len(charges) != 6 or len(discharges) != 6:
        raise ValueError("CSV 结果行数异常，请先运行 solve_q1_milp.py。")

    # 先复制模板，再只改动允许填写的数值单元格，保留工作表、样式和 A 列时间标签。
    shutil.copy2(template_path, result_path)
    workbook = load_workbook(result_path)
    if workbook.sheetnames != ["计划购电量", "充放电量"]:
        raise ValueError("官方 result1.xlsx 模板的工作表结构不符合预期。")
    ...
    if final_labels != original_labels:
        raise RuntimeError("官方模板的时间标签发生了意外变化。")
