...
SOC_MIN = 1200.0
SOC_MAX = 10800.0
PERIOD_LIMIT = 5000.0 * DT_HOUR 
# CBC 以有限小数写回变量；
# 1e-3kWh 远小于输入的 0.0001kW 的精度及10分钟转换误差。
TOL = 1e-3
...
def main() -> None:
    ...
    # 原始首行0:10 对应 00:00--00:10；
    # 末行0:00+1 对应 23:50--24:00
    ...
    for t in periods:
        # 能量平衡公式：购电 + 光伏 - 弃光 + 放电 = 负荷 + 充电。
        model += (
            grid[t] + pv_kwh[t] - curtail[t] + discharge[t]
            == load_kwh[t] + charge[t]
        ), f"energy_balance_{t:03d}"
        model += curtail[t] <= pv_kwh[t], f"curtailment_limit_{t:03d}"
        # SOC递推：C、D均为交流母线侧电量。
